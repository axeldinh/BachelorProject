...
function rhs = create_rhs_volume_strong(rhs, q, q_ref, grid_obj, visc, tol,o)
    ngl = grid_obj.ngl;
    nel = grid_obj.nel;
    xgl = grid_obj.xgl;
    ...
    for ie=1:nel
        ...
        for i=1:ngl
            ...
            if (idry == 1)
                il = i; ir = i; vl = il; vr = ir; %--I created vl vr to handle when il<1 and ir>ngl
                while(vl>1)
                    if(q(1,il-1,ie) > tol && (ir-il)<2)
                        il = il -1;
                        vl = il;
                    else
                        vl = 0;
                ...
                if(il~=ir)
                    b_e = Deriv(xgl(il:ir),q_ref(1,il:ir,ie),i-il+1); %--We take i-il+1 to point to the correct index
                    r_e = Deriv(xgl(il:ir),q(1,il:ir,ie),i-il+1);
                end
...
%--Creates Finite Differences of Arbitrary order
function df = Deriv(x,f,i)
%--Approximates f'(x_i) with sum(a_j*f(x_i + b_j*h)) / h
...
%--Recover the coefficients such that x_j = x_i + b_j*h
b = zeros(1,n);
for j = 1:n
    b(j) = (x(j)-x(i))/h;
end
